...
from planemotion.core.menu_system import MenuManager
...
class PlaneMotionView:
    ...
    def __init__(self, viewmodel: 'PlaneMotionViewModel', width: int = 1280, height: int = 720):
        ...
        import os
        # Find theme.json in the package
        ...
        try:
            if os.path.exists(theme_path):
                ...
                print("Warning: theme.json not found, menu text may not be visible")
                self.gui_manager = pygame_gui.UIManager((width, height))
        except FileNotFoundError as e:
            print(f"Warning: pygame-gui font loading failed: {e}")
            # Try with theme but disable live updates
            if os.path.exists(theme_path):
                self.gui_manager = pygame_gui.UIManager((width, height), theme_path, enable_live_theme_updates=False)
            else:
                self.gui_manager = pygame_gui.UIManager((width, height), enable_live_theme_updates=False)
